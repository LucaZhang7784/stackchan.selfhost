...
class ASRProviderBase(ABC):
    ...
    async def handle_voice_stop(self, conn: "ConnectionHandler", asr_audio_task: List[bytes]):
        ...
        try:
            ...
            if text_len > 0:
                # 唤醒词回声拦截: ASR 把「阿松/你好小智」识别成用户消息时忽略,
                # 不触发 LLM 回复(设备已在聆听, 用户马上说真正的话; 不拦截会抢答)
                _, wake_clean = remove_punctuation_and_length(content_for_length_check)
                if wake_clean in ("阿松", "你好小智"):
                    logger.bind(tag=TAG).info(f"忽略唤醒词回声: {content_for_length_check}")
                    return
