...
class TTSProviderBase(ABC):
    ...
    def to_tts_stream(self, text, opus_handler: Callable[[bytes], None] = None) -> None:
        # 保留原始文本用于显示/上报
        original_text = text
        # Phase 7.1: EdgeTTS 流式推流 - 边合成边出帧, 不等整句
        streamer = getattr(self, "stream_to_opus", None)
        if streamer is not None and getattr(self, "streaming_enabled", True):
            sid = getattr(self, "current_sentence_id", None)
            first_buf = []       # 预积攒 3 帧(180ms)防网络微抖动 Pop
            first_sent = [False]

            def _on_frame(opus_data: bytes):
                if not first_sent[0]:
                    first_buf.append(opus_data)
                    if len(first_buf) >= 3:
                        self.tts_audio_queue.put((SentenceType.FIRST, None, original_text, sid))
                        for f in first_buf:
                            opus_handler(f)
                        first_buf.clear()
                        first_sent[0] = True
            ...
            ok = False
            try:
                streamer(text, sid, _on_frame)
                if not first_sent[0] and first_buf:  # 极短句: 流结束后补发
                    self.tts_audio_queue.put((SentenceType.FIRST, None, original_text, sid))
                    for f in first_buf:
                        opus_handler(f)
                    first_buf.clear()
                ...
                logger.bind(tag=TAG).warning(f"流式TTS失败(已发帧={first_sent[0]}): {e}")
                ok = first_sent[0]  # 已发帧则视为成功, 避免重复播报; 未发则回退非流式
            if ok:
                return None
    ...
    def tts_one_sentence(
        ...
        # Phase 7.1: 首句流式分切 - 标点优先(Unicode), 无标点/Markdown 长句按字数强制兜底,
        # 防止 LLM 输出无标点长句时 TTS 首包死等; 切分按 Unicode 字符数, 严禁按字节切(避免截断 UTF-8)
        MAX_TTS_CHUNK_CHARS = 24  # 无标点时的强制切分长度(可调)
        text = content_detail or ""
        try:
            text = MarkdownCleaner.clean_markdown(text)
        except Exception:
            pass
        # Phase 7.1: 标点含逗号(，,、), 保证 "好的，" 这类首句 100ms 内触发首包 TTS
        raw = re.split(r"([。！？!?；;，,、\n])", text)
        segments = []
        for part in raw:
            if not part:
            ...
            if part in "。！？!?；;，,、\n":
                if segments:
                    segments[-1] += part  # 标点粘回前句, 保持语气
                continue
            # 字数强制兜底: 按 Unicode 字符切
            while len(part) > MAX_TTS_CHUNK_CHARS:
                segments.append(part[:MAX_TTS_CHUNK_CHARS])
                part = part[MAX_TTS_CHUNK_CHARS:]
            if part:
