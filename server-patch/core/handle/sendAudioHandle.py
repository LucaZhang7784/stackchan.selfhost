...
async def sendAudioMessage(conn: "ConnectionHandler", sentenceType, audios, text, sentence_id=None):
    ...
    if not conn.calling and sentenceType == SentenceType.LAST:
        # Phase 7.1 终极修补: 最后一包 opus 发出后等 20ms 再发 stop,
        # 确保尾包已被 ESP32 成功入队(配合固件 EOF: stop 后队列排空才切待机)
...
async def _send_audio_with_rate_control(
    ...
    """
    Phase 7.1 终极修补: 3 帧小批次微休眠 (Batch Pacing) 平滑推流。
    严禁对每个 60ms Opus 包盲目 sleep(0.04) —— OS 定时器漂移(62ms)会饿死喇叭。
    每 3 帧(180ms 音频)让出 10ms: 帧突发让 ESP32 本地队列积攒缓冲,
    吸收 Tailscale/WARP 网络抖动, 既防 DMA 溢出又防喇叭断续。

    Args:
        conn: 连接对象
        audio_list: 音频包列表
        rate_controller: 速率控制器(兼容保留, Batch Pacing 不再逐帧 pacing)
        flow_control: 流控状态
        send_delay: 固定延迟（秒），-1表示使用动态流控
    """
    BATCH_SIZE = 3
    for i, packet in enumerate(audio_list):
        if conn.client_abort:
            return
        ...
        await _do_send_audio(conn, packet, flow_control)
        if (i + 1) % BATCH_SIZE == 0:
            await asyncio.sleep(0.01)  # 每 180ms 音频让出 10ms
